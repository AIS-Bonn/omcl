...
class SAMClipRGBEncoder:
    def __init__(self, querys):
        with torch.no_grad():
            self.device = 'cuda'
            self.use_openclip = False
            self.use_prompt_ensemble = False
            self.batch_size = 1024
            self.top_k = 1
            self.threshold = 0.1
            self.clip = OpenCLIP() if self.use_openclip else OpenAICLIP()
            self.downsample = 2
            self.resolution = 1
            self.segany = AutoSegmentAnything(os.path.join(os.path.expanduser("~"), 'data/models/SAM'), "vit_h", self.device)
            self.segclip = AutoSegAnyCLIP(self.segany, self.clip, self.device)
            assert(self.top_k >= 1)
            assert(self.resolution > 0 and self.resolution <= 3)
            self.querys = querys
            self.querys_features = self.clip.encode_text(self.querys).cpu()

    def forward(self, rgb_image):
        with torch.no_grad():
            image = cv2.resize(rgb_image, (rgb_image.shape[1]//self.downsample, rgb_image.shape[0]//self.downsample), interpolation=cv2.INTER_AREA)
            self.segany.set_image(image)
            self.segclip.n_objs = self.resolution
            image_features = self.segclip.encode_image(extent_segmentation_mask=1, bbox_crop=False)
            # del segany, segclip
            # torch.cuda.empty_cache()
            # similarity_argmax_top_k = self.clip.predict_similarity_objects_with_feature_attention_batch(image_features, self.querys, self.use_prompt_ensemble, self.batch_size, self.top_k, self.threshold)
            # similarity_argmax_top_k  = [similarity_argmax.cpu() for similarity_argmax in similarity_argmax_top_k]
            similarity_mask_down = self.clip.predict_similarity_objects_with_feature_attention_batch(image_features, self.querys, self.use_prompt_ensemble, self.batch_size, self.top_k, self.threshold)[0].numpy()
            similarity_mask = cv2.resize(similarity_mask_down, rgb_image.shape[:2],  interpolation=cv2.INTER_NEAREST) 
            ...
            return self.querys_features[similarity_mask], similarity_mask+1

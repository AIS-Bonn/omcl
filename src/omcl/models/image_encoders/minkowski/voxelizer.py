...
import collections
import numpy as np
from omcl.models.image_encoders.minkowski.voxelization_utils import sparse_quantize
...
class Voxelizer:
    ...
    def voxelize(self, coords, feats, labels, center=None, link=None, return_ind=False):
        ...
        if return_ind:
            return coords_aug, feats, np.array(inds_reconstruct), inds
        if link is not None:
            return coords_aug, feats, np.array(inds_reconstruct), link[inds]
